...
@app.route('/api/asset-class-detail')
def api_asset_class_detail():
    """
    순자산 차트의 자산군 카드(주식·ETF/현금/부동산/코인/연금) 클릭 시
    해당 자산군을 구성하는 개별 항목과 평가금액을 반환.
    daily_snapshots 집계와 동일한 기준으로 값을 계산해 카드에 표시된 합계와 일치시킨다.
    """
    ...
    if category == 'stocks':
        ...
        for r in cur.fetchall():
            ...
            items.append({'name': r['name'], 'sub': '주식', 'value': value})
        cur.close()
        ...
        for r in cur.fetchall():
            ...
            mul = ex_rate if is_foreign_ticker(r['ticker']) else 1.0
            value = round(qty * _sf(r['current_price'])) * mul
            items.append({'name': r['name'], 'sub': 'ETF', 'value': value})
        cur.close()

    elif category == 'cash':
        _refresh_usd_cash_amounts(db, ex_rate)
        cur = db.cursor()
        cur.execute("SELECT name, amount FROM cash_deposits ORDER BY amount DESC")
        for r in cur.fetchall():
            items.append({'name': r['name'], 'sub': '현금', 'value': _sf(r['amount'])})
        cur.close()
        ...
        if residence_deposit:
            items.append({'name': '거주 보증금', 'sub': '현금성 자산', 'value': residence_deposit})

    elif category == 'real_estate':
        cur = db.cursor()
        cur.execute("SELECT name, re_type, current_price FROM real_estate ORDER BY current_price DESC")
        for r in cur.fetchall():
            items.append({'name': r['name'], 'sub': r['re_type'] or '부동산', 'value': _sf(r['current_price'])})
        cur.close()

    elif category == 'crypto':
        cur = db.cursor()
        cur.execute("SELECT name, symbol, current_price, quantity FROM crypto")
        for r in cur.fetchall():
            value = _sf(r['current_price']) * _sf(r['quantity'])
            items.append({'name': r['name'], 'sub': r['symbol'] or '코인', 'value': value})
        cur.close()

    elif category == 'pension':
        cur = db.cursor()
        cur.execute("SELECT name, pension_type, institution, accumulated FROM pension")
        for r in cur.fetchall():
            items.append({'name': r['name'], 'sub': r['institution'] or r['pension_type'] or '연금', 'value': _sf(r['accumulated'])})
        cur.close()
    ...
    db.close()
    items = [it for it in items if it['value']]
